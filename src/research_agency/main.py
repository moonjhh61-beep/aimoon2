"""CLI entry point for ResearchAgency."""
...
from .agency import ResearchAgency, ResearchAgencyConfig
from .config import (
    API_KEY_ENV_VAR,
    DEFAULT_KEY_PATH,
    DEFAULT_PROMPT_PATH,
...
REQUEST_HEADING = "### 연구 요청"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run ResearchAgency once or in interactive mode.")
    ...
    add_user_request_arguments(parser, label="research brief")
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.7,
...
def create_agency(
    ...
    *,
    ...
    llm_client = LLMClient(api_key=api_key, agency_name="ResearchAgency")
    config = ResearchAgencyConfig(
    ...
    return ResearchAgency(llm_client, config)


def run_interactive(agency: ResearchAgency) -> None:
    print("ResearchAgency ready. Type your messages, or press Ctrl+C to exit.")
    try:
        while True:
            ...
            print(f"ResearchAgency > {response}\n")
