"""CLI entry point for BacktestAgency."""
...
REQUEST_HEADING = "### 백테스트 요청"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BacktestAgency once or in interactive mode.")
    ...
    add_user_request_arguments(parser, label="backtest brief")
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.7,
...
def create_agency(
    ...
    *,
    ...
    return BacktestAgency(llm_client, config)


def run_interactive(agency: BacktestAgency) -> None:
    print("BacktestAgency ready. Type your messages, or press Ctrl+C to exit.")
    try:
        while True:
            ...
            print(f"BacktestAgency > {response}\n")
