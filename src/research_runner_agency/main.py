"""CLI entry point for ResearchRunnerAgency."""
...
from .agency import ResearchRunnerAgency, ResearchRunnerAgencyConfig
from .config import (
    API_KEY_ENV_VAR,
    DEFAULT_KEY_PATH,
    DEFAULT_PROMPT_PATH,
...
REQUEST_HEADING = "### 러너 요청"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run ResearchRunnerAgency once or in interactive mode.")
    ...
    add_user_request_arguments(parser, label="runner brief")
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.7,
...
def create_agency(
    ...
    *,
    ...
    llm_client = LLMClient(api_key=api_key, agency_name="ResearchRunnerAgency")
    config = ResearchRunnerAgencyConfig(
    ...
    return ResearchRunnerAgency(llm_client, config)


def run_interactive(agency: ResearchRunnerAgency) -> None:
    print("ResearchRunnerAgency ready. Type your messages, or press Ctrl+C to exit.")
    try:
        while True:
            ...
            print(f"ResearchRunnerAgency > {response}\n")
